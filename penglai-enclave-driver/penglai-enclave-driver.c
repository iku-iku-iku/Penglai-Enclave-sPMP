...
int enclave_ioctl_init(void)
{
    ...
    /* [Dd] Should we broadcast some states (e.g., PT_area region) to other
     * harts? */
    /* [LX] sm will broadcast */
    addr = __get_free_pages(GFP_HIGHUSER, DEFAULT_SECURE_PAGES_ORDER);
    if (!addr) {
        printk("[Penglai KModule]: can not get free page which order is 0x%x",
...
#if 1
    sbiret = SBI_CALL_2(SBI_SM_INIT, __pa(addr),
                        1 << (DEFAULT_SECURE_PAGES_ORDER + RISCV_PGSHIFT));
    ret = sbiret.value;
    // if(ret < 0)
    if (sbiret.error) {
...
void enclave_ioctl_exit(void)
{
    ...
    while (addr) {
        ...
        if (count != (*size) && (*size > 0)) {
            printk(
                "KERNEL MODULE:  the number of free pages is not exponential "
                "times of two\n");
            kfree(size);
            return;
        }
        ...
        sbiret =
            SBI_CALL_2(SBI_SM_FREE_ENCLAVE_MEM, __pa(size), FREE_MAX_MEMORY);
