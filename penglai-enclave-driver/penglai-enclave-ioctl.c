...
// now we just acqure a big lock before allocating enclave mem, and release the
// lock after initializing mem and returning it back to sm
...
unsigned int total_enclave_page(int elf_size, int stack_size)
{
    ...
    total_pages = PAGE_UP(elf_size) / RISCV_PGSIZE +
                  PAGE_UP(stack_size) / RISCV_PGSIZE + 15;
    return total_pages;
}

int create_sbi_param(enclave_t *enclave,
                     struct penglai_enclave_sbi_param *enclave_sbi_param,
                     unsigned long paddr, unsigned long size,
                     unsigned long entry_point, unsigned long untrusted_ptr,
                     unsigned long untrusted_size, unsigned long free_mem)
{
    enclave_sbi_param->eid_ptr = (unsigned int *)__pa(&enclave->eid);
    enclave_sbi_param->ecall_arg0 =
        (unsigned long *)__pa(&enclave->ocall_func_id);
    ...
    enclave_sbi_param->ecall_arg3 =
        (unsigned long *)__pa(&enclave->ocall_syscall_num);
...
int alloc_untrusted_mem(unsigned long untrusted_mem_size,
                        unsigned long *untrusted_mem_ptr, enclave_t *enclave)
{
    int ret = 0;
    vaddr_t addr;
    ...
    *untrusted_mem_ptr = addr;
    map_untrusted_mem(enclave->enclave_mem, DEFAULT_UNTRUSTED_PTR, __pa(addr),
                      untrusted_mem_size);

    return ret;
}

int alloc_kbuffer(unsigned long kbuffer_size, unsigned long *kbuffer_ptr,
                  enclave_t *enclave)
{
    int ret = 0;
    vaddr_t addr;
    ...
    *kbuffer_ptr = addr;
    map_kbuffer(enclave->enclave_mem, ENCLAVE_DEFAULT_KBUFFER, __pa(addr),
                kbuffer_size);
                           ...
                           long untrusted_mem_size)
{
    if ((elf_size > MAX_ELF_SIZE) || (stack_size > MAX_STACK_SIZE) ||
        (untrusted_mem_size > MAX_UNTRUSTED_MEM_SIZE))
        return -1;
    return 0;
}
...
int penglai_enclave_create(struct file *filep, unsigned long args)
{
    struct penglai_enclave_user_param *enclave_param =
        (struct penglai_enclave_user_param *)args;
    ...
    long untrusted_mem_size =
        enclave_param->untrusted_mem_size;  // DEFAULT_UNTRUSTED_SIZE=8KB
    unsigned long untrusted_mem_ptr = enclave_param->untrusted_mem_ptr;  // 0
    unsigned long kbuffer_ptr = ENCLAVE_DEFAULT_KBUFFER;
    struct penglai_enclave_sbi_param *enclave_sbi_param =
        kmalloc(sizeof(struct penglai_enclave_sbi_param), GFP_KERNEL);
    enclave_t *enclave;
    ...
    if (penglai_enclave_eapp_preprare(enclave->enclave_mem, elf_ptr, elf_size,
                                      &elf_entry, STACK_POINT,
                                      stack_size))  // May sleep
    {
    ...
    if ((untrusted_mem_ptr == 0) && (untrusted_mem_size > 0)) {
        alloc_untrusted_mem(untrusted_mem_size, &untrusted_mem_ptr,
                            enclave);  // May sleep
    }
    enclave->untrusted_mem->addr = (vaddr_t)untrusted_mem_ptr;
    enclave->untrusted_mem->size = untrusted_mem_size;
    printk(
        "[Penglai Driver@%s] untrusted_mem->addr:0x%lx "
        "untrusted_mem->size:0x%lx\n",
        __func__, (vaddr_t)untrusted_mem_ptr, untrusted_mem_size);

    alloc_kbuffer(ENCLAVE_DEFAULT_KBUFFER_SIZE, &kbuffer_ptr,
                  enclave);  // May sleep
    ...
    create_sbi_param(enclave, enclave_sbi_param,
                     ...
                     enclave->enclave_mem->size, elf_entry,
                     DEFAULT_UNTRUSTED_PTR, untrusted_mem_size, __pa(free_mem));
...
int handle_memory_extend(enclave_t *enclave)
{
    ...
    addr = __get_free_pages(GFP_HIGHUSER, order);
    if (!addr) {
        printk("KERNEL MODULE: can not get free pages which order is 0x%lx",
               order);
        return -1;
    }
...
int penglai_enclave_run(struct file *filep, unsigned long args)
{
    ...
    if ((void *)untrusted_mem_ptr != NULL && untrusted_mem_size > 0) {
        ...
        memset((void *)enclave->untrusted_mem->addr, 0,
               enclave->untrusted_mem->size);
        if (copy_from_user((void *)enclave->untrusted_mem->addr,
                           (void *)untrusted_mem_ptr, untrusted_mem_size))
            return -EFAULT;
    }
    ...
    while ((ret.value == ENCLAVE_TIMER_IRQ) || (ret.value == ENCLAVE_OCALL)) {
        if (ret.value == ENCLAVE_TIMER_IRQ) {
            schedule();
            ret = SBI_CALL_3(SBI_SM_RESUME_ENCLAVE, enclave->eid,
                             RESUME_FROM_TIMER_IRQ,
                             get_cycles64() + DEFAULT_CLOCK_DELAY);
        }
        else {
            ...
            case OCALL_SYS_WRITE: {
                ...
                ret = SBI_CALL_3(SBI_SM_RESUME_ENCLAVE, resume_id,
                                 RESUME_FROM_OCALL, OCALL_SYS_WRITE);
                break;
            }
            case OCALL_USER_DEFINED: {
                ...
                if ((void *)untrusted_mem_ptr != NULL && ocall_buf_size > 0) {
                    if (ocall_buf_size > enclave->untrusted_mem->size) {
                        printk(
                            "KERNEL MODULE: untrusted memory is not big enough "
                            "\n");
                        return -EINVAL;
                    }
                    if (copy_to_user((void *)untrusted_mem_ptr,
                                     (void *)enclave->untrusted_mem->addr,
                                     ocall_buf_size))
                        return -EFAULT;
                }
            ...
            default: {
                ret = SBI_CALL_2(SBI_SM_RESUME_ENCLAVE, resume_id,
                                 RESUME_FROM_OCALL);
            }
            }
        }
...
{
    struct penglai_enclave_ioctl_attest_enclave *enclave_param =
        (struct penglai_enclave_ioctl_attest_enclave *)args;
    ...
    ret = SBI_CALL_3(SBI_SM_ATTEST_ENCLAVE, enclave->eid, __pa(report),
                     enclave_param->nonce);
    ...
    case USER_PARAM_RESUME_FROM_CUSTOM_OCALL: {
        ...
        if ((void *)untrusted_mem_ptr != NULL && ocall_buf_size > 0) {
            ...
            memset((void *)enclave->untrusted_mem->addr, 0,
                   enclave->untrusted_mem->size);
            if (copy_from_user((void *)enclave->untrusted_mem->addr,
                               (void *)untrusted_mem_ptr, ocall_buf_size)) {
                retval = -EFAULT;
                return -EFAULT;
            }
        }
        ret = SBI_CALL_3(SBI_SM_RESUME_ENCLAVE, enclave->eid, RESUME_FROM_OCALL,
                         OCALL_USER_DEFINED);
        resume_id = enclave->eid;

        while ((ret.value == ENCLAVE_TIMER_IRQ) ||
               (ret.value == ENCLAVE_OCALL)) {
            if (ret.value == ENCLAVE_TIMER_IRQ) {
                schedule();
                ret = SBI_CALL_3(SBI_SM_RESUME_ENCLAVE, enclave->eid,
                                 RESUME_FROM_TIMER_IRQ,
                ...
                case OCALL_USER_DEFINED: {
                    ocall_buf_size = enclave->ocall_arg0;
                    if ((void *)untrusted_mem_ptr != NULL &&
                        ocall_buf_size > 0) {
                        if (ocall_buf_size > enclave->untrusted_mem->size) {
                            printk(
                                "KERNEL MODULE: untrusted memory is not big "
                                "enough \n");
                            return -EINVAL;
                        }
                        if (copy_to_user((void *)untrusted_mem_ptr,
...
long penglai_sign(struct file*filep, unsigned long args) {
    acquire_big_lock(__func__);
    struct sbiret ret = SBI_CALL_1(SBI_SM_SIGN, args);
    release_big_lock(__func__);
    return ret.value;
}

long penglai_enclave_ioctl(struct file *filep, unsigned int cmd,
                           unsigned long args)
{
    ...
    case PENGLAI_ENCLAVE_IOC_DEBUG_PRINT:
        ...
        ret = sbiret.value;
        break;
    case PENGLAI_ENCLAVE_IOC_SIGN:
        ret = penglai_sign(filep, (unsigned long)ioctl_data);
        break;
    default:
        return -EFAULT;
    }
